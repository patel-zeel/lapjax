...
class Prior:
    def __init__(self, distributions, bijectors):
        ...
        self.transformed_distributions = jax.tree_map(
            lambda _, bijector, dist: tfd.TransformedDistribution(
                dist, tfb.Invert(bijector)
            ),
            self.guide,
            self.bijectors,
            self.distributions,
    ...
    def log_prob(self, sample):
        log_probs = jax.tree_map(
            lambda value, dist: dist.log_prob(value),
            sample,
    ...
    def sample(self, seed, sample_shape=()):
        ...
        return jax.tree_map(
            lambda seed, dist: dist.sample(seed=seed, sample_shape=sample_shape),
            seeds,
            self.transformed_distributions,
        )
...
class Variational:
    def __init__(self, prior, bijectors, vi_type="full_rank"):
        ...
        def get_flat_shape(dist):
            if isinstance(dist, tfd.CholeskyLKJ):
                assert (
                    dist.batch_shape.as_list() == []
                ), "CholeskyLKJ multi-batch is not supported"
        ...
        self.shapes = jax.tree_map(
            lambda _, dist: get_shape(dist), self.guide, self.prior.distributions
        )
        self.flat_shapes = jax.tree_map(
            lambda _, dist: get_flat_shape(dist), self.guide, self.prior.distributions
        )
        ...
        def get_variational(bijector, flat_shape):
            if self.vi_type == "mean_field":
                normal_dist = tfd.MultivariateNormalDiag(
                    loc=jnp.zeros(flat_shape), scale_diag=jnp.ones(flat_shape)
                )
            elif self.vi_type == "full_rank":
                normal_dist = tfd.MultivariateNormalTriL(
                    loc=jnp.zeros(flat_shape), scale_tril=jnp.eye(flat_shape)
                )
    ...
    def transform_dist(self, dist):
        ...
        values = jax.tree_map(
            lambda value, transform: transform(value), values, self.params_transforms
        )
    ...
    def sample(self, seed, sample_shape=()):
        ...
        seeds = seeds_like(seed, self.guide)
        return jax.tree_map(
            lambda seed, dist, shape: sample_dist(seed, dist, sample_shape, shape),
            seeds,
            self.params,
            self.shapes,
        )
    ...
    def log_prob(self, sample):
        ...
        log_probs = jax.tree_map(
            lambda tmp_sample, dist, flat_shape: log_prob_dist(
                dist, tmp_sample, flat_shape
            ),
            sample,
            self.params,
            self.flat_shapes,
